...
tid_t process_fork(const char *name, struct intr_frame *if_ UNUSED)
{
    ...
    sema_down(&child->load_sema);  //! __do_fork가 실행되어 load될때까지 부모는 대기
...
struct thread *get_child_process(int pid) {
    ...
    for (e; e != list_end(child_list); e = list_next(e)) {
...
load(const char *file_name, struct intr_frame *if_)
{
    ...
    char *parse[LOADER_ARGS_LEN]; // 128
    int count = 0;
    char *save_ptr, *token;
    ...
    argument_stack(parse, count, &if_->rsp); // 함수 내부에서 parse와 rsp의 값을 직접 변경하기 위해 주소 전달
	if_->R.rdi = count;
...
int process_add_file(struct file *f) {
    ...
    // for (curr->next_fd; curr->next_fd < FDT_COUNT_LIMIT; curr->next_fd++) {
    //     if (!fdt[curr->next_fd]) {
    //         curr->next_fd += 1;
